...
- A test of the update system: this release exists only to check that the app
  notices a new version, updates itself, and shows you this notice. Nothing about
  designing symbols has changed.
...
- Fixed: the "What's new" notice could list the wrong releases. This app and
  Bliss Tiles and Puzzles shared a single record of which release you last saw,
  so opening one could make the other's notice show releases you had already
  read, or skip the notice altogether. Each app now keeps its own record. The
  same applied to the "Remind me in a week" postponement on a designer file
